...
function invokePayload(options, command, cooked, remain) {
    ...
    // console.log(command, " hshhshs ", options);
    if (
        command.DETAILS.payload &&
        !hasCommandInOptions(command.DETAILS.commands, options)
    ) {
